...
class Form extends React.Component {
  constructor() {
    super();
    this.state = { result: " ", function: " ", userInput: [] };
  }
  update = (event) => {
    let value = event.target.value;
    this.setState({
      userInput: value.split(",").map((item) => Number(item)),
    });
  };
  ...
  functions = (event) => {
    ...
    switch (this.state.function) {
      case "sum":
        this.setState({
          result: this.state.userInput.reduce((a, b) => a + b),
        });
        break;
      case "average":
        this.setState({
          result:
            this.state.userInput.reduce((a, b) => a + b) /
      ...
      case "mode":
        let mode = [];
        let numbers = {};
        this.state.userInput.forEach((num) =>
